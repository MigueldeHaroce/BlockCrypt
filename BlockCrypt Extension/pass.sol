...
contract Keychain {
    ...
    mapping(address => bool) private hasId; // New mapping to track if a user already has an ID
    ...
    function setKey(string memory key, string memory value) public {
        ...
        require(!hasId[msg.sender], "User already has an ID"); // Check if the user already has an ID
